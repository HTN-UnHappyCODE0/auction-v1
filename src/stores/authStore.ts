	...
	actions: {
		...
		async loginUser(form: Record<string, string>) {
			return new Promise<String>(async (resolve, reject) => {
				try {
					const {data} = await axios.post<APIResponse<{accessToken: string; refreshToken: string}>>('/login', {...form});
		...
		async refreshToken() {
			return new Promise<string>(async (resolve, reject) => {
				try {
					const {data} = await axios.post<APIResponse<{accessToken: string; refreshToken: string}>>('/refresh');
